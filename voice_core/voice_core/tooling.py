...
DEFAULT_SYSTEM_PROMPT = (
    ...
    "Без Markdown, списков, эмодзи и блоков кода, если их прямо не просили. "
    "Короткие предложения и пунктуация для пауз. Не ставь метки вроде [Омни]. "
    "Не выводи think, reasoning, analysis и внутренние рассуждения. "
    "Ответ озвучивается синтезатором. "
    ...
    "Нужные факты и действия бери через инструменты, не выдумывай свежие данные. "
    "Перед вызовом инструмента можешь коротко сказать нейтральную фразу "
    "(«Секунду», «Сейчас посмотрю») — она озвучится сразу. "
    "Чтобы закончить голосовой диалог, вызови end_conversation; при желании "
    "произнеси короткое прощание в том же ходе."
)
...
END_CONVERSATION_TOOL: dict[str, object] = {
    ...
    "function": {
        ...
        "description": (
            ...
            "Вызывай при любой явной просьбе закончить, замолчать, отключиться "
            "или перестать слушать. Не проси специальное слово. "
            "Можешь передать короткое прощание в farewell. "
